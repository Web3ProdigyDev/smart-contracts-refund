...
pub fn record_kill_handler(
    ...
    require!(killer_team != victim_team, WagerError::InvalidKillTarget);

    // CRITICAL FIX: Validate killer and victim are different players
    require!(killer != victim, WagerError::InvalidKillTarget);
    ...
    let victim_spawns = match victim_team {
        0 => {
            require!(
                victim_index < game_session.team_a.player_spawns.len(),
                WagerError::InvalidPlayer
            );
        ...
        1 => {
            require!(
                victim_index < game_session.team_b.player_spawns.len(),
                WagerError::InvalidPlayer
            );
    ...
    let killer_kills = match killer_team {
        0 => {
            require!(
                killer_index < game_session.team_a.player_kills.len(),
                WagerError::InvalidPlayer
            );
        ...
        1 => {
            require!(
                killer_index < game_session.team_b.player_kills.len(),
                WagerError::InvalidPlayer
            );
    ...
    require!(killer_kills < u8::MAX, WagerError::ArithmeticError);
    ...
    msg!(
        "Kill recorded successfully: Player {} (team {}) killed Player {} (team {})",
        killer,
        killer_team,
        victim,
        victim_team
    ...
    msg!(
        "Updated stats - Killer kills: {} -> {}, Victim spawns: {} -> {}",
        original_killer_kills,
        new_killer_kills,
        original_victim_spawns,
        new_victim_spawns
    );
